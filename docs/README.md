...
> **現在地** — Phase 0、M-01、M-02は完了しています。seed 0のB64/B256/B1024各100epoch、共通PCA、両背景の損失平面、GIFペアをD:側で確認済みです。次はM-03としてseed 1・2を追加します。
...
| [ToDo List](TODO.md) | 依存関係、担当、完了条件、進捗 | M-01/M-02完了、次はM-03 |
...
Phase 1に必要な設定、データ、初期化、学習、再開、射影、損失平面、GIF生成は実装済みです。M-01/M-02のseed 0最小版はD:側で完了しました。実行手順と成果物はルートREADMEの[M-01](../README.md#m-01-seed-0の3run)に記載し、共有splitと共通初期重みを全runで再利用します。

## 残る実装と確認

- M-03としてseed 1・2の6runを追加し、全9runの共通PCA、seed別および全run summaryの両背景GIF、条件別評価表を完成すること。
